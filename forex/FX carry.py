'''
    Title: FX carry
    Description: Carry strategy for FX. Among G10, pick top 2 to go long and bottom 2 to 
    go short, based on interest rates differentials. Re-balance every month
    Style tags: Risk Factor, Carry
    Asset class: FX
    Dataset: FXCM Minute
'''
import pandas as pd
...
def compute_rollovers(context, data):
    print('rolling over {}'.format(data.current_dt))
    ...
    print('{} carry {}'.format(data.current_dt,daily_carry))
