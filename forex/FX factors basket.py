'''
    Title: FX factors basket
    Description: Basket strategy for FX with equal allocation to value, momentum and
        carry.
    Style tags: Risk Factor, Smart Beta
    Asset class: FX
    Dataset: FXCM Minute
'''
import pandas as pd
...
def signal_function_momentum(context, data):
    lookback = context.params['lookback']
    fx_data = data.history(context.universe,'close',312,'1d')
    fx_vols = fx_data.pct_change(periods=1).std()*(312**0.5)
    momentum = {}
    for pair in context.universe:
        momentum[pair] = ((fx_data[pair][-1]/fx_data[pair][-lookback])**(312/lookback) - 1)/fx_vols[pair]
    
    context.momentum_weights = {}
    n = len(context.universe)
    for k in momentum.items():
        if k[1] < -0.5:
            context.momentum_weights[k[0]] = -1.0/n
        elif k[1] > 0.5:
            context.momentum_weights[k[0]] = 1.0/n
        else:
            context.momentum_weights[k[0]] = 0.0
...
def signal_function_valuation(context, data):
    ...
    context.valuation_weights = dict(short_pos+zero_pos+long_pos)
...
def signal_function_carry(context, data):
    ...
    context.carry_weights = dict(short_pos+zero_pos+long_pos)
...
def rebalance(context,data):
    ...
    '''
    signal_function_momentum(context, data)
    signal_function_valuation(context, data)
    signal_function_carry(context, data)
    
    for security in context.universe:
        leverage = context.params['leverage']
        weight = context.momentum_weights[security] + context.valuation_weights[security] + context.carry_weights[security]
        order_target_percent(security, leverage*weight/3.0)
