'''
    Title: FX Momentum (Timeseries)
    Description: Timeseries momentum strategy for FX. Among G10, pick top 2 to go long 
        and bottom 2 to go short, based on volatility-adjusted past returns. Re-balance 
        every month
    Style tags: Momentum, Risk Factor
    Asset class: FX
    Dataset: FXCM Minute
'''
import pandas as pd
...
def signal_function_momentum(context, data):
    ...
    for k in momentum.items():
        if k[1] < -0.5:
            context.weights[k[0]] = -1.0/n
        elif k[1] > 0.5:
            context.weights[k[0]] = 1.0/n
        else:
            context.weights[k[0]] = 0.0
