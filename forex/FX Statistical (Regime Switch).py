'''
    ...
    Description: Use any indicator to create base signal. Use change point analysis 
        on accumulated profits to determine regime switch using bootstrapping.
...
def initialize(context):
    ...
    context.params = {'verbose':False,
                      ...
                      'SMA_period_short':500,       
                      'SMA_period_long':1000,     
    ...
    context.pnls = []
    context.change_point_flag = 1
    context.change_point_stat = 0
    context.change_point_signal = 1
...
def before_trading_start(context, data):
    ...
    context.pnls.append(context.portfolio.pnl)
    if len(context.pnls) < (max_len+1):
        return
    
    pnls = context.pnls[-(max_len+1):]
    returns = [j-i for i, j in zip(pnls[:-1], pnls[1:])]
    sim_metrics = []
    metric = max(pnls) - min(pnls)
    
    for i in range(n_sims):
        sim_returns = np.random.choice(returns, max_len)
        sim_pnls = np.cumsum(sim_returns)
        sim_metrics.append(max(sim_pnls) - min(sim_pnls))

    z = [m < metric for m in sim_metrics]
    test_stat = float(sum(z))/len(z)
    slope = ((pnls[-1] - pnls[0])/max_len)/np.std(returns)
    
    if context.change_point_stat < 0.4 and test_stat > 0.4:
        context.change_point_flag = 1
    else:
        context.change_point_flag = 0

    if context.change_point_flag == 1:
        context.change_point_signal = -context.change_point_signal
    elif slope < -1.0:
        context.change_point_signal = -context.change_point_signal

    context.change_point_stat = test_stat
    #print('pnl {} slope {} metric {} stat {} signal {}'.format(context.pnls[-1], slope, metric, context.change_point_flag,context.change_point_signal))
...
def calculate_signal(px, params):
    ...
    '''
    ind1 = sma(px,params['SMA_period_short'])
    ind2 = sma(px,params['SMA_period_long'])
    
    return np.sign(ind1-ind2)
...
def signal_function(context, data):
    ...
    for security in context.universe:
        ...
        context.signals[security] = calculate_signal(px, context.params)*context.change_point_signal
