...
def initialize(context):
    # strategy parameters
    context.params = {'universe':'NIFTY-I,BANKNIFTY-I',
                      'stoploss':0.005,
                      'takeprofit':0.02,
                      'short_sma':10,
                      'long_sma':30,
    ...
    schedule_function(strategy, date_rules.every_day(),
                      time_rules.marke_open(hours=0, minutes=30))
    schedule_function(stop_entry, date_rules.every_day(),
                      time_rules.marke_open(hours=1, minutes=30))
    schedule_function(square_off_all, date_rules.every_day(),
                      time_rules.market_close(minutes=30))
    
def get_previous_range(context, data):
    cols = ['high','low']
