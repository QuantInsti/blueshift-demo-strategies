'''
    ...
    Description: And ensemble of multiple technical strategies. Each individual 
        strategies are modelled as expert advise. An agent assigns weights to each 
        strategy periodically. This allows a range of allocation scheme.
...
class Agent():
    ...
    def weighing_function(self):
        ...
        weights = dict((key,1.0/self.n_advisors) for key in self.advisors_keys)
        #weights = dict(zip(self.advisors_keys,[1 if i==3 else 0 for i in range(self.n_advisors)]))
